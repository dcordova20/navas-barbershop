...
function ensure() {
  ...
  if (!fs.existsSync(AUTH_PATH)) writeJson(AUTH_PATH, JSON.stringify({ users: {} }, null, 2));
  if (!fs.existsSync(CONTENT_PATH)) writeJson(CONTENT_PATH, JSON.stringify({
  ...
  if (!fs.existsSync(SESSIONS_PATH)) writeJson(SESSIONS_PATH, JSON.stringify({}, null, 2));
}
...
function requireAuth(req, res) {
  ...
  if (!s) { sendJson(res, 401, { error: "Auth required" }); return null; }
  return s;
}
...
const server = http.createServer((req, res) => {
  try {
    ...
    if (pathname === "/api/content" && req.method === "GET") {
      sendJson(res, 200, readJson(CONTENT_PATH) || {});
      return;
    }
    ...
    if (pathname === "/api/content" && req.method === "PUT") {
      ...
      req.on("end", () => {
        let data;
        try { data = JSON.parse(Buffer.concat(body).toString()); } catch (e) {
    ...
    if (pathname === "/api/publish" && req.method === "POST") {
      ...
      if (!content.services || !content.services.items || !content.services.items.length) missing.push("services.items");
    ...
    if (pathname === "/api/auth/register" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          auth.users[email] = { passwordHash: crypto.createHash("sha256").update(password).digest("hex") };
          writeJson(AUTH_PATH, auth);
          const token = makeToken();
          const sessions = readJson(SESSIONS_PATH) || {};
          sessions[token] = { email, createdAt: Date.now() };
          writeJson(SESSIONS_PATH, sessions);
          res.writeHead(200, { "Set-Cookie": "navas_session=" + encodeURIComponent(token) + "; Path=/; HttpOnly" });
          sendJson(res, 200, { ok: true });
        } catch (e) {
    ...
    if (pathname === "/api/auth/login" && req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          if (hash !== user.passwordHash) { sendJson(res, 401, { error: "Invalid credentials" }); return; }
          const token = makeToken();
          const sessions = readJson(SESSIONS_PATH) || {};
          sessions[token] = { email, createdAt: Date.now() };
          writeJson(SESSIONS_PATH, sessions);
    ...
    if (pathname === "/api/auth/me" && req.method === "GET") {
      const s = getSession(req);
      if (!s) { sendJson(res, 401, { error: "Not logged in" }); return; }
    ...
    if (pathname === "/api/images" && req.method === "GET") {
      const list = fs.readdirSync(IMAGES_DIR).filter((f) => /\.(jpg|jpeg|png|gif|webp)$/i.test(f));
    ...
    if (pathname === "/api/upload" && req.method === "POST") {
      ...
      req.on("end", () => {
        ...
        if (!match) { sendJson(res, 400, { error: "no file" }); return; }
        ...
        fs.writeFileSync(path.join(IMAGES_DIR, filename), filebuf);
